...
int main(int argc, char** argv) {
  ...
  label.push_back("aeroplane");
  label.push_back("bicycle");
  label.push_back("bird");
  label.push_back("boat");
  label.push_back("bottle");
  label.push_back("bus");
  label.push_back("car");
  label.push_back("cat");
  label.push_back("chair");
  label.push_back("cow");
  label.push_back("diningtable");
  label.push_back("dog");
  label.push_back("horse");
  label.push_back("motorbike");
  label.push_back("person");
  label.push_back("pottedplant");
  label.push_back("sheep");
  label.push_back("sofa");
  label.push_back("train");
  label.push_back("tvmonitor");
    ...
    if (file_type == "image") {
      ...
      for(int i =0; i< results.size();i++){
        ...
        if(results[i][results[i][4] + 6] >confidence ){
          fout<<file<<" "<< label[results[i][4] + 1]<<" "<<results[i][6 + results[i][4]]<<" "<<xmin<<" "<<ymin<<" "<<xmax<<" "<<ymax<<endl;
          LOG(INFO)<<file<<" "<< label[results[i][4]]<<" "<<results[i][6 + results[i][4]]<<" "<<xmin<<" "<<ymin<<" "<<xmax<<" "<<ymax<<endl;
          //fout<<file<<" "<< results[i][4]<<" "<<results[i][6 + results[i][4]]<<" "<<xmin<<" "<<ymin<<" "<<xmax<<" "<<ymax<<endl;
         cv::rectangle(img,cvPoint(xmin,ymin),cvPoint(xmax,ymax),cv::Scalar(255,0,255),2,1,0);
         // LOG(INFO)<<file<<" "<< results[i][4]<<" "<<results[i][6 + results[i][4]]<<" "<<xmin<<" "<<ymin<<" "<<xmax<<" "<<ymax<<endl;
         }
      }
