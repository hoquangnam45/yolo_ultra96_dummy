...
void detect(vector<vector<float>> &boxes, vector<float> result,
    ...
    //vector<float> biases{30,61, 62,45, 59,119, 10,13, 16,30, 33,23};
    ...
    for (int h = 0; h < height; ++h) {
        for (int w = 0; w < width; ++w) {
            for (int c = 0; c < anchorCnt; ++c) {
                ...
                //cout << biases[2 * c + 2 * anchorCnt * num] << endl;
                box.push_back(-1);
...
vector<vector<float>> applyNMS(vector<vector<float>>& boxes,int classes, const float thres) {
    ...
    for(int k = 0; k < classes; k++) {
        ...
        for (size_t _i = 0; _i < boxes.size(); ++_i) {
            ...
            //     if (ovr >= thres) 
            //         if (boxes[i][5] < boxes[j][5]) exist_box[i] = false;
            //         else exist_box[j] = false;
            // }
...
image load_image_cv(const cv::Mat& img) {
    ...
    //     for (int k = 0; k < c; k++){
    //         for (int j = 0; j < w; j++){
    //             //if (i*w*im.c + j*im.c + k >= 416 * 416 * 3) cout << i*w*im.c + j*im.c + k << endl;
    //             test.data[i*w*c + j*c + k] = (unsigned char) (im.data[k*w*h + i*w + j] * 255);
    //         }
    //     }
    // }
    // imwrite("revert_yolo_input.jpg", test);
    return im;
}
...
image letterbox_image(image im, int w, int h)
{
    ...
    // Mat test(Size(w, h), CV_8UC3);
    // cout << boxed.w << " " << boxed.h << " " << boxed.c << endl;
    // for(int i = 0; i < boxed.w*boxed.h; ++i){
    //     float swap = boxed.data[i];
    //     boxed.data[i] = boxed.data[i+boxed.w*boxed.h*2];
    //     boxed.data[i+boxed.w*boxed.h*2] = swap;
    // }
    ...
    //         for (int j = 0; j < w; j++){
    //             //if (i*w*im.c + j*im.c + k >= 416 * 416 * 3) cout << i*w*im.c + j*im.c + k << endl;
    //             test.data[i*w*im.c + j*im.c + k] = (uint8_t) (boxed.data[k*w*h + i*w + j] * 255);
    //         }
    //     }
    // }
    // imwrite("yolo_input.jpg", test);
    free_image(resized);
